    ...
    long long cal_memory(const std::vector<int>& granularity, int layer) {
        ...
        for (int i = subgraphs[layer].size() - 1; i >= 0; i--) {
            ...
            if (i == subgraphs[layer].size() - 1) {
                ...
                if (h[nd.outputs[0]][0] > native_granularity[0] || w[nd.outputs[0]][0] > native_granularity[1]) {
                    return INF_LANTENCY;
                }
                if (h[nd.outputs[0]][1] > native_granularity[0] || w[nd.outputs[0]][1] > native_granularity[1]) {
                    return INF_LANTENCY;
                }
                if (nd.is_matmul) {
                    for (int j = 0; j <= 1; j++) {
    ...
    double get_lantency(int layer) {
        ...
        for (int i = 0; i < m; i++) {
            for (int k = 0; k < m_k; k++) {
                ...
                lantency += std::max((turn_memory + link_h_memory) / slow_memory_bandwidth, cal_lantency) * h_num;
                lantency += std::max((turn_memory + link_w_memory) / slow_memory_bandwidth, cal_lantency) * w_num;
            }
        }
        return lantency;
    }
