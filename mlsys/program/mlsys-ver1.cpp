...
public:
    ...
    float upd_traversal_order(int layer) {
    ...
    bool upd_granularity(int layer) {
        ...
        int H_lim = std::min(native_granularity[0], tensors[nd.outputs[0]].height);
        int W_lim = std::min(native_granularity[1], tensors[nd.outputs[0]].width);
    ...
    float get_lantency(int layer) {
        ...
        for (int i = 0; i < subgraphs[layer].size(); i++) {
            ...
            for (int j = 0; j < nd.inputs.size(); j++) {
                int id = nd.inputs[j];
                if (d_in[layer][id] == 0  && i != retained_input) {
                    if (h[id][0] > h[id][1] && w[id][0] < w[id][1]) {
                        global_memory += h[id][0] * w[id][0] + h[id][1] * w[id][1];
                    } else {
                        global_memory += std::max(h[id][0], h[id][1]) * std::max(w[id][0], w[id][1]);
    ...
    std::tuple<std::vector<Tensor>, std::vector<bool>, float> sa_solution() {
        ...
        float init_temp_0 = 90000.0 * n;
        float min_temp_0 = 0.1;
        float cooling_rate_0 = 0.98;
        for (float temp_0 = init_temp_0; temp_0 >= min_temp_0;) {
            ...
            if (!get_subgraphs()) { tensors[x].is_cut ^= 1; }
                ...
                if (!get_granularities()) { tensors[x].is_cut ^= 1;}
                    ...
                    float init_temp_1 = 3400.0 * n;
                    float min_temp_1 = 0.1;
                    float cooling_rate_1 = 0.8;
                    for (float temp_1 = init_temp_1; temp_1 >= min_temp_1; temp_1 *= cooling_rate_1) {
                        if (n_layers > 1) {
                            ...
                            float temp_lantency[2];
                            std::vector<int> temp_granularty[2];
                            std::vector<int> temp_traversal_order[2];
                            ...
                            float new_lantency = current_lantency;
