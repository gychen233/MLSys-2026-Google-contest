...
public:
    ...
    void upd_granularity(long long layer) {
    ...
    double compute_lantency(Node nd, int H, int W, int K) {
        if (nd.is_matmul) {
            return (double)nd.cost * std::max(H / native_granularity[0], 1ll)
                                   * std::max(W / native_granularity[1], 1ll)
    ...
    std::tuple<double, std::vector<bool> > get_retain() {
        ...
        std::vector<long long> temp_pre = {0, 0};
        for (int layer = 0; layer < n_layers; layer++) {
            if (layer == 0) {
                retain[layer] = 0;
                upd_retain(layer);
        ...
        assert(lantency[0] <= lantency[1]); // delete
...
void solve(const std::string file_in_name, const std::string file_out_name) {
    ...
    for (long long t = 0; t < 3; t++) {
        auto [current_lantency, current_cut] = 
    ...
    auto [subgraphs, granularities, tensors_to_retain, traversal_orders, lantencies] =
            myEvaluater.final_solution(best_cut);
