...
struct TensorNode {
    ...
    bool is_input;
    TensorNode(): height(0), width(0), input(0), is_cut(0), is_retained(0), is_input(0) {}
};
...
void printT(TensorNode t) {
    ...
    printf("\n");
    printf("is_cut: %d, is_retained: %d, is_input: %d\n", t.is_cut, t.is_retained, t.is_input);
    printf("\n");
}
            ...
            for (int i = subgraphs[layer].size() - 1; i >= 0; i--) {
                ...
                for (int j = 0; j < nd.outputs.size(); j++) {
                    ...
                    while(single_compute_size[output_id].size()) {
                        ...
                        // printf("@%d %d\n", output_id, single_compute_size[output_id].size());
                        single_compute_size[output_id].pop_back();
                        // printf("@%d %d\n", output_id, single_compute_size[output_id].size());
                        // printf("!%d %d %d\n", output_id, dat.height, dat.width);
                        temp_memory = 0;
                        if (nd.is_matmul) {
                            ...
                            for (int l = 0; l < nd.inputs.size(); l++) {
                                if (d_in[layer][nd.inputs[l]]) {
                                    ...
                                    // printf("%d %d %d\n", nd.inputs[l], dat.height, dat.width);
                                    // printf("?%d\n", single_compute_size[16].size());
                                }
                                else {
                                    if (!retained_tensor[nd.inputs[l]]) {
                                        temp_memory += dat.height * dat.width;
                                        // printf("PointWise: %d %d %d %d\n", nd.inputs[l], dat.height, dat.width, temp_memory);
                                    }
                                }
                            }
                        }
                        if (std::max(retain_memory + output_memory + temp_memory, conflict_memory) > fast_memory_capacity) {
                            printf("layer: %d\n", layer);
                            printf("retain_memory: %d\n", retain_memory);
                            printf("output_memory: %d\n", output_memory);
                            printf("temp_memory: %d\n", temp_memory);
                            printf("conflict_memory: %d\n", conflict_memory);
...
private:
    ...
    ScheduleEvaluator myEvaluater;
    std::vector<bool> granularity_order;
public:
    ComputeGraph (
        const std::vector<int>& widths,
        const std::vector<int>& heights,
    ...
    ComputeGraph (
        ...
        const std::vector<int>& native_granularity,
        ScheduleEvaluator& myEvaluater,
        const std::vector<TensorNode> tensors,
        const std::vector<bool> granularity_order
    ) {
        this->fast_memory_capacity = fast_memory_capacity;
        this->slow_memory_bandwidth = slow_memory_bandwidth;
        this->native_granularity = native_granularity;
        ...
        this->granularity_order = granularity_order;
...
#undef cal_memory
    ...
    float evaluate_lantency(std::vector<std::vector<int> >& subgraphs,
                            ...
                            std::vector<std::vector<int> >& tensors_to_retain,
                            std::vector<std::vector<int> >& traversal_orders) {
        std::vector<float> lantencies = myEvaluater.evaluate(subgraphs,
                                                            granularities,
                                                            tensors_to_retain,
                                                            traversal_orders);
        if (lantencies.size() == 0) {
            return INF_LANTENCY;
        }
        float total_lantency = 0.;
        for (int i = 0; i < lantencies.size(); i++) {
            total_lantency += lantencies[i];
        }
    ...
    float evaluate_lantency_of_cut() {
        auto [subgraphs, granularities, tensors_to_retain, traversal_orders] =
                    get_solution();
        if (subgraphs.size() == 0) {
            return INF_LANTENCY;
        }
    ...
    std::tuple<std::vector<std::vector<int> >,
               ...
               std::vector<std::vector<int> >,
               std::vector<std::vector<int> >,
               std::vector<float> > final_solution() {
        auto [subgraphs, granularities, tensors_to_retain, traversal_orders] =
                    get_solution();
        std::vector<float> lantencies = myEvaluater.evaluate(subgraphs,
                                                             granularities,
    ...
    std::tuple<std::vector<TensorNode>, std::vector<bool>, float> sa_solution() {
        std::vector<std::vector<int> > subgraphs;
        std::vector<std::vector<int> > granularities;
        std::vector<std::vector<int> > tensors_to_retain;
        std::vector<std::vector<int> > traversal_orders;
        float best_lantency, current_lantency;
        std::vector<TensorNode> best_state;
        std::vector<bool> best_granularity_order;
        best_state.resize(n);
        best_granularity_order.assign(m, 0);
        for (int i = 0; i < n; i++) {
            tensors[i].is_cut = 1;
            tensors[i].is_retained = 0;
            best_state[i] = tensors[i];
        }
        granularity_order.assign(m, 0);
        ...
        for (float temp = init_temp; temp >= min_temp;) {
            ...
            if (flag == 2) {
                granularity_order[y] = !granularity_order[y];
            }
            ...
            if (current_lantency > INF_LANTENCY / 2) {
                ...
                if (ran_real(0, 1) < std::exp((best_lantency - current_lantency) / temp)) {
                    if (current_lantency < best_lantency) {
                        best_state = tensors;
                        best_granularity_order = granularity_order;
        ...
        printf("%lf\n", best_lantency);
        return {best_state, best_granularity_order, best_lantency};
    }
};
...
void solve(const std::string file_in_name, const std::string file_out_name) {
    ...
    for (int t = 0; t < 10; t++) {
        ...
        auto [currrent_state, currrent_granularity_order, currrent_lantency] = 
                                    myComputeGraph.sa_solution();
        if (currrent_lantency < best_lantency) {
            best_state = currrent_state;
            best_granularity_order = currrent_granularity_order;
            best_lantency = currrent_lantency;
        }
    }
...
void check_out(const std::string file_in_name, const std::string file_out_name) {
    std::ifstream file_in(file_in_name);
    if (!file_in.is_open()) {
    ...
    std::vector<std::vector<int> > granularities = j_out["granularities"];
    std::vector<std::vector<int> > tensors_to_retain = j_out["tensors_to_retain"];
    std::vector<std::vector<int> > traversal_orders = j_out["traversal_orders"];
    std::vector<float> subgraph_latencies = j_out["subgraph_latencies"];
